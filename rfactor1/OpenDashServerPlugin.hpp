...
class OpenDashPlugin : public InternalsPluginV3
{
protected:
...
public:
	static const long rFactorVersion; // '1', this code provides rFactor 1 structures
	...
	void UpdateGraphics(const GraphicsInfoV2 &info) { return; };

	// GAME INPUT
	bool HasHardwareInputs() { return(false); } // CHANGE TO TRUE TO ENABLE HARDWARE EXAMPLE!
	void UpdateHardware(const float fDT) { return; } // update the hardware with the time between frames
	void EnableHardware() { return; }             // message from game to enable hardware
	void DisableHardware() { return; }           // message from game to disable hardware
